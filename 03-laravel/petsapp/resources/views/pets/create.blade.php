...
@section('content')
    ...
    <form method="post" action="{{ route('pets.store') }}" class="pt-4 pb-20" enctype="multipart/form-data">
        ...
        <fieldset class="fieldset w-md bg-base-200 border border-base-300 p-4 rounded-box">
            ...
            <input type="file" name="imagen" id="image" class="hidden" accept="image/*">
...
@section('js')
    <script>
        ...
        const image = document.querySelector('#image'); // Cambiado de imagen a imagen
        const preview = document.querySelector('#preview');

        upload.addEventListener('click', function(e) {
            image.click(); // Cambiado de imagen.click() a imagen.click()
        });

        image.addEventListener('change', function(e) { // Cambiado de imagen.addEventListener a imagen.addEventListener
